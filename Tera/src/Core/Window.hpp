...
/**
 * @brief Tera Window Interface
 * 
 * This is basically a reimplementation of the NVPWindow class
 */
class Window
{
...
public:
    ...
    // returns false on exit, can do while(pollEvents()){ ... }
    bool pollEvents()                   { TeraSystem::pollEvents(); return !isClosing(); }
    void waitEvents()                   { TeraSystem::waitEvents(); }
    double getTime()                    { return TeraSystem::getTime(); }
    std::string exePath()               { return TeraSystem::exePath(); }

    // accessors
    int getWidth()          const       { return m_windowSize[0]; }
    int getHeight()         const       { return m_windowSize[1]; }
    ...
    void setKeyModifiers(int m)         { m_keyModifiers = m; }
    void setMouse(int x, int y)         { m_mouseX = x; m_mouseY = y; }

    bool isFullScreen()     const       { return m_isFullScreen; }
    bool isClosing()        const       { return m_isClosing || glfwWindowShouldClose(m_internal); }
    bool isOpen()           const       { return glfwGetWindowAttrib(m_internal, GLFW_VISIBLE) == GLFW_TRUE && glfwGetWindowAttrib(m_internal, GLFW_ICONIFIED) == GLFW_FALSE && !isClosing(); }

    // creates internal window and opens it
    virtual bool open(int posX, int posY, int width, int height, const char *title, bool requireGLContext);
    // destroys internal window
    void deinit();

    //  triggers closing event, still needs deinit for final cleanup
    void close();
    void maximize();
    void restore();
    void minimize();
    ...
    // the app needs to refresh once the window
    virtual void requestPaint() {}
    // the app might swith on/off the continuous rendering
    virtual void requestContinuousRefresh(bool bYes) {}
...
private:
    static void onWindowRefreshCb(GLFWwindow *glfwwin);
    static void onWindowSizeCb(GLFWwindow *glfwwin, int w, int h);
    static void onWindowCloseCb(GLFWwindow *glfwwin);
    static void onMouseButtonCb(GLFWwindow *glfwwin, int button, int action, int mods);
    static void onCursorPosCb(GLFWwindow *glfwwin, double x, double y);
    static void onScrollCb(GLFWwindow *glfwwin, double x, double y);
    static void onKeyCb(GLFWwindow *glfwwin, int key, int scancode, int action, int mods);
    static void onCharCb(GLFWwindow *glfwwin, unsigned int codepoint);
    static void onDropCb(GLFWwindow *glfwwin, int count, const char **paths);
